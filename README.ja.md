...
システムは、**AI Therapist** (`rfs_therapist`) がエージェントを制御するクローズドループ・サイクルで動作します。**人間（ユーザー）** はオプションで家族の対話に介入し、シミュレーションに影響を与えることができます。
...
| ノード | 役割 | 主な機能 |
| :--- | :--- | :--- |
| **`rfs_therapist`** | 中央オーケストレーター | 勾配降下法を用いて治療ターゲットを計算します。 |
...
| **`move`** | Integer | `1` | toioの移動を有効化: `1` (有効), `0` (無効)。 |
